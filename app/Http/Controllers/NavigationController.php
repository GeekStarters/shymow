	...
	public function viewUser($name = null){
		if ($name != null) {
			$users = Perfil::select('*','perfils.id as user_id')
								...
								->where('perfils.id',$name)
								->where('perfils.active',true)->first();
			// dd($users);
			...
			return view('logueado.ver_perfil',compact('users','posts','socialNet','streamNet','dataUsers'));
			}else{
				return redirect('/');
			}
		}else{
